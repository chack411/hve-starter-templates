...
1. Treat `execute` as the frontmatter tool set that exposes a terminal-execution tool, not as a shell command named `execute`. Invoke the available terminal tool; do not decide that PowerShell is unavailable merely because no tool is literally named `PowerShell` or `execute`.
2. Before the first edit or review action, invoke the terminal tool. On Windows, run this exact PowerShell command:
	...
	On Linux or macOS, run a platform-equivalent command that returns both local datetime with UTC offset and Unix epoch milliseconds. Do not use the conversation date, model knowledge, file timestamps, or an estimated time.
3. Accept the result only when the tool call actually ran successfully, `datetime` matches `YYYY-MM-DD HH:mm:ss ±HH:mm`, and `epochMilliseconds` is numeric. Immediately append the start row to the output document and `docs/project-status.md`; keep the epoch value in working context for elapsed-time calculation.
4. If the first Windows command actually returns an execution or parsing error, retry once through the terminal tool with `Get-Date -Format "yyyy-MM-dd HH:mm:ss zzz"`. Do not skip the first call or claim likely incompatibility without its real tool result.
...
8. Mark the current timestamp as `時刻未記録` only when the terminal-execution tool is absent from the runtime, or when an actual primary call and its fallback both return errors. Record which calls were attempted and their errors, set duration to `未記録`, and report that the time-recording requirement failed. Never use `時刻未記録` merely because command support is uncertain. Preserve it for historical entries whose actual time cannot be recovered.
